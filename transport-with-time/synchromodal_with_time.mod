...
//-------------------------------
float traffic_condition[Nodes]; // 表示当前节点交通情况，值越大表示越拥堵
float weather_condition[Nodes]; // 表示当前节点的天气状况，值越大表示天气越恶劣
int mode_availability[Modes]; // 每种运输模式的可用性，0表示不可用，1表示可用
...
minimize 
    sum(gd in GoodsDetails, s in Services) s.transitcost * x[gd.good][s] * (1 + traffic_condition[s.origin] + weather_condition[s.origin]) // 运输成本 Transportation cost 
...
subject to {
    ...
    //-----------------------------------
    forall(gd in GoodsDetails, s in Services) {
    if (mode_availability[s.modality] == 0) {
        x[gd.good][s] == 0;  // 如果运输方式不可用，则不能选择此服务
    }
}
